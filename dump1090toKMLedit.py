...
def load_dump1090():
    ...
    with open(fileName, newline='') as csvfile:   # <--  Replaced 'logs.csv' with 'fileName'         ~TheCanfield
...
def createKML(KMLdict, ICAO):
    ...
    with open("./output/" + name + ".kml", "w") as kmlfile:   # <-- Added './output/'    ~TheCanfield
